...
> **Every message to a student goes through `sendNudge`** (`src/lib/nudge-delivery.ts`): a Teams chat from **Neram Assistant** first, never from a teacher's own Teams (founder, 2026-09-24; a teacher action passes `teacher`/`chat`/`sendAs` and the Assistant card gets "From Hari" plus a "Message Hari" button; `src/lib/sender-classification.test.ts` holds this), the activity feed only when no chat landed, the Nexus bell always, `{name}`/`{firstName}` auto-filled, a receipt per recipient. No email channel. Never group-post inactivity. `src/lib/notification-door-guard.test.ts` lists the routes still to migrate; a new direct write fails it.
