...
> **Every message to a student goes through `sendNudge`** (`src/lib/nudge-delivery.ts`): a Teams chat first (the teacher's own chat via `teacherChatFrom` when they press Send, with the browser sending `getTeacherToken()`; for anything automatic, `sendAs` the classroom's connected teacher from `src/lib/teams-sender.ts`), the activity feed only when no chat landed, the Nexus bell always, `{name}`/`{firstName}` auto-filled, a receipt per recipient. No email channel. Never group-post inactivity. `src/lib/notification-door-guard.test.ts` lists the routes still to migrate; a new direct write fails it.
