...
interface FoundationChapterListContentProps {
  /** URL prefix for chapter links. Defaults to '/student/foundation' */
  chapterLinkPrefix?: string;
  /** Hide the PageHeader (when embedded in a parent page that already has one) */
  hideHeader?: boolean;
}

export default function FoundationChapterListContent({
  chapterLinkPrefix = '/student/foundation',
  hideHeader = false,
}: FoundationChapterListContentProps) {
                ...
                <ChapterCard
                  ...
                  onClick={() => {
                    if (effectiveStatus !== 'locked') {
                      router.push(`${chapterLinkPrefix}/${chapter.id}`);
                    }
                  }}
                />
              );
