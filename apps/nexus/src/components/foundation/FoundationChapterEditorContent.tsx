...
interface SectionWithCount extends NexusFoundationSection {
  question_count: number;
  min_questions_to_pass: number | null;
}

interface FoundationChapterEditorContentProps {
  chapterId: string;
  /** URL for the back button. Defaults to '/teacher/foundation/manage' */
  backUrl?: string;
}

export default function FoundationChapterEditorContent({
  chapterId,
  backUrl = '/teacher/foundation/manage',
}: FoundationChapterEditorContentProps) {
  const theme = useTheme();
  const router = useRouter();
  const { getToken, loading: authLoading } = useNexusAuthContext();

  const [chapter, setChapter] = useState<NexusFoundationChapter | null>(null);
  ...
  const [sectionQuestions, setSectionQuestions] = useState<Record<string, NexusFoundationQuizQuestion[]>>({});
  ...
  const fetchChapter = useCallback(async () => {
    ...
    try {
      ...
      if (!token) return;
      const res = await fetch(`/api/foundation/admin/chapters/${chapterId}`, {
      ...
      if (res.ok) {
        ...
        setForm({
          ...
          video_source: data.chapter.video_source || 'youtube',
          youtube_video_id: data.chapter.youtube_video_id || '',
          sharepoint_video_url: data.chapter.sharepoint_video_url || '',
          video_duration_seconds: data.chapter.video_duration_seconds || 0,
  ...
  const handleAddSection = async () => {
    ...
    try {
      const token = await getToken();
      if (!token) return;
      const res = await fetch(`/api/foundation/admin/chapters/${chapterId}/sections`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const handleDeleteSection = async (sectionId: string) => {
    try {
      const token = await getToken();
      if (!token) return;
      const res = await fetch(`/api/foundation/admin/sections/${sectionId}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` },
      });
      if (res.ok) {
  ...
  const fetchQuestions = async (sectionId: string) => {
    ...
    try {
      const token = await getToken();
      if (!token) return;
      const res = await fetch(`/api/foundation/admin/sections/${sectionId}/questions`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      if (res.ok) {
        const data = await res.json();
  ...
  const handleDeleteQuestion = async (questionId: string, sectionId: string) => {
    try {
      const token = await getToken();
      if (!token) return;
      const res = await fetch(`/api/foundation/admin/questions/${questionId}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` },
      });
      if (res.ok) {
  ...
  const handleAutoGenerate = async (vttContent?: string) => {
    setAutoGenerating(true);
    setAutoGenStatus(vttContent ? 'Analyzing transcript & generating sections...' : 'Fetching transcript...');
    try {
      const token = await getToken();
      if (!token) return;

      setAutoGenStatus('Analyzing transcript & generating sections...');
      const res = await fetch(`/api/foundation/admin/chapters/${chapterId}/auto-generate`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
        body: JSON.stringify({
          sharepoint_video_url: form.sharepoint_video_url,
          title: form.title,
          ...(vttContent ? { vtt_content: vttContent } : {}),
        }),
      });
      ...
      if (data.error === 'no_transcript') {
        setAutoGenSnackbar({ open: true, message: data.message || 'No transcript found. Try uploading a .vtt file instead.', severity: 'warning' });
        return;
      }
  ...
  const handleVttFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const handleJsonFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      ...
      try {
        ...
        setAutoGenSnackbar({ open: true, message: 'Failed to parse JSON file. Make sure it is valid JSON.', severity: 'error' });
      }
    };
    reader.readAsText(file);
    e.target.value = '';
  ...
  const handleAcceptGenerated = async (accepted: GeneratedSection[]) => {
    ...
    try {
      ...
      const res = await fetch(`/api/foundation/admin/chapters/${chapterId}/auto-generate/save`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
  ...
  if (!chapter) {
    return (
      <Box sx={{ textAlign: 'center', py: 6 }}>
        <Typography variant="h6" sx={{ color: 'text.secondary' }}>Chapter not found</Typography>
      </Box>
    );
  }
    ...
    <Box>
      ...
      <Paper variant="outlined" sx={{ p: 2, mb: 2, borderRadius: 2.5 }}>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 0.5 }}>
              ...
              <IconButton
                ...
                onClick={async () => {
                  ...
                  if (!token) return;
                  const res = await fetch(
                    `/api/foundation/admin/chapters/${chapterId}/audio-tracks?trackId=${track.id}`,
                    { method: 'DELETE', headers: { Authorization: `Bearer ${token}` } }
                  );
                  if (res.ok) {
        ...
        <FileUploadZone
          ...
          uploadUrl={`/api/foundation/admin/chapters/${chapterId}/audio-tracks`}
          maxSizeMB={100}
          getToken={getToken}
          onUpload={(data) => {
        ...
        {audioTracks.length > 0 && (
          <Typography variant="caption" color="text.secondary" sx={{ mt: 1, display: 'block' }}>
            Upload another file to add a different language version.
          </Typography>
        )}
      </Paper>
                        ...
                        <TextField
                          ...
                          onChange={async (e) => {
                            ...
                            try {
                              const token = await getToken();
                              if (!token) return;
                              await fetch(`/api/foundation/admin/sections/${section.id}`, {
                                method: 'PATCH',
                                headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
                            ...
                            <Box sx={{ flex: 1 }}>
                              ...
                              <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5 }}>
                                {['a', 'b', 'c', 'd'].map(opt => (
                                  <Chip
                                    key={opt}
                                    label={`${opt.toUpperCase()}: ${(q as any)[`option_${opt}`]}`}
                                    size="small"
                                    sx={{
                                      height: 22,
                                      fontSize: '0.65rem',
