...
export interface StatFilterTile {
  key: ResultFilter;
  label: string;
  value: number;
  hint: string;
...
export default function StudentStatFilters({
  tiles,
  active,
  onChange,
}: {
  tiles: StatFilterTile[];
  active: ResultFilter;
  ...
  return (
    <Box
      ...
      sx={{
        ...
        gridTemplateColumns: { md: `repeat(${tiles.length}, minmax(0, 1fr))` },
        gap: 1,
        mb: 1.5,
    ...
    >
      {tiles.map((t) => {
        ...
        const empty = t.value === 0 && t.key !== 'all';
        return (
          <Box
            component="button"
            type="button"
            ...
            onClick={() => onChange(selected && t.key !== 'all' ? 'all' : t.key)}
          ...
          >
            ...
            <Typography
              ...
              noWrap
              sx={{ display: 'block', maxWidth: '100%', lineHeight: 1.3 }}
            >
              {t.hint}
            </Typography>
          </Box>
