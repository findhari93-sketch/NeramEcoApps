...
export async function POST(
  ...
  try {
    ...
    const { data: chapter } = await supabase
      ...
      .select('pdf_storage_path, pdf_onedrive_item_id')
      .eq('id', chapterId)
      .single();

    if (chapter?.pdf_onedrive_item_id) {
      await deleteFromSharePoint(token, chapter.pdf_onedrive_item_id).catch(() => {});
    }

    const timestamp = Date.now();
    const filePath = `nexus/chapters/${chapterId}/pdf/${timestamp}.pdf`;
