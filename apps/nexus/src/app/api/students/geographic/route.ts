 ...
 *
 * Population is the "active" set only (current + future exam years, alumni excluded).
 * See apps/nexus/src/lib/geo-students.ts for the shared predicate.
 */
export async function GET(request: NextRequest) {
  try {
    ...
    if (search && search.trim()) {
      ...
      const result = searchActiveStudents(students, search.trim(), { limit, offset });
      return NextResponse.json(result);
    }

    // Hierarchy mode (default).
    const hierarchy = buildHierarchy(students);
    ...
    if (stateFilter && filtered.length > 0) {
      filtered = filtered.map((c) => ({
        ...c,
        states: c.states.filter((s) => s.state.toLowerCase() === stateFilter.toLowerCase()),
      }));
    }
