 ...
 *
 * Only `channel_meeting` classes can be repaired: their event lives on the
 * team's group calendar and has a real attendee list. A standalone
 * /me/onlineMeetings link has no Outlook attendee collection to rewrite, so
 * those are reported as skipped rather than silently counted as done.
 *
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: classes } = await supabase
      ...
      .select(
        'id, title, scheduled_date, teacher_id, teams_meeting_id, teams_meeting_scope, classroom_id, status',
      )
    ...
    for (const cls of upcoming as any[]) {
      ...
      if (cls.teams_meeting_scope !== 'channel_meeting') {
        skipped.push({ ...label, reason: 'Standalone meeting link has no attendee list to rewrite' });
        continue;
      }
      ...
      try {
        const res = await fetch(
          `https://graph.microsoft.com/v1.0/groups/${teamId}/calendar/events/${cls.teams_meeting_id}`,
          {
            method: 'PATCH',
