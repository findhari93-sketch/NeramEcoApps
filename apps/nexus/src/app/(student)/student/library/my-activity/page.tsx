'use client';

import { useState, useEffect, useCallback } from 'react';
import {
  Box,
  Typography,
  Paper,
  Skeleton,
  LinearProgress,
  alpha,
  useTheme,
} from '@neram/ui';
import PlayCircleOutlineIcon from '@mui/icons-material/PlayCircleOutline';
import CheckCircleOutlinedIcon from '@mui/icons-material/CheckCircleOutlined';
import VideocamOutlinedIcon from '@mui/icons-material/VideocamOutlined';
import BookmarkOutlinedIcon from '@mui/icons-material/BookmarkOutlined';
import { useNexusAuthContext } from '@/hooks/useNexusAuth';
import StreakBanner from '@/components/library/engagement/StreakBanner';
import WeeklyActivityDots from '@/components/library/engagement/WeeklyActivityDots';

interface MyActivityData {
  streak: {
    current_streak_days: number;
    best_streak_days: number;
    engagement_status: string;
    engagement_score: number;
    total_active_days: number;
  };
  total_videos_available: number;
  videos_watched_count: number;
  videos_completed_count: number;
  weekly_activity: { date: string; watched: boolean; watch_seconds: number }[];
  watch_time_this_week: number;
  continue_watching: ContinueWatchingItem[];
  bookmarks: BookmarkItem[];
}

interface ContinueWatchingItem {
  video_id: string;
  last_position_seconds: number;
  total_watched_seconds: number;
  completed: boolean;
  video: {
    id: string;
    approved_title: string | null;
    suggested_title: string | null;
    youtube_thumbnail_url: string | null;
    duration_seconds: number | null;
  } | null;
}

interface BookmarkItem {
  id: string;
  video_id: string;
  timestamp_seconds: number | null;
  note: string | null;
  video: {
    approved_title: string | null;
    suggested_title: string | null;
  } | null;
}

function formatDuration(seconds: number): string {
  if (seconds <= 0) return '0m';
  const h = Math.floor(seconds / 3600);
  const m = Math.floor((seconds % 3600) / 60);
  if (h > 0 && m > 0) return `${h}h ${m}m`;
  if (h > 0) return `${h}h`;
  return `${m}m`;
}

function formatTimestamp(seconds: number): string {
  const m = Math.floor(seconds / 60);
  const s = Math.floor(seconds % 60);
  return `${m}:${s.toString().padStart(2, '0')}`;
}

export default function MyActivityPage() {
  const theme = useTheme();
  const { getToken } = useNexusAuthContext();
  const [data, setData] = useState<MyActivityData | null>(null);
  const [loading, setLoading] = useState(true);

  const fetchActivity = useCallback(async () => {
    setLoading(true);
    try {
      const token = await getToken();
      if (!token) return;

      const res = await fetch('/api/library/engagement/me', {
        headers: { Authorization: `Bearer ${token}` },
      });

      if (res.ok) {
        const json = await res.json();
        setData(json.data);
      }
    } catch (err) {
      console.error('Failed to load activity:', err);
    } finally {
      setLoading(false);
    }
  }, [getToken]);

  useEffect(() => {
    fetchActivity();
  }, [fetchActivity]);

  if (loading) {
    return (
      <Box>
        <Typography variant="subtitle1" sx={{ fontWeight: 700, mb: 2 }}>
          My Activity
        </Typography>
        <Skeleton variant="rounded" height={80} sx={{ borderRadius: 2.5, mb: 2 }} />
        <Skeleton variant="rounded" height={60} sx={{ borderRadius: 2, mb: 2 }} />
        <Skeleton variant="rounded" height={50} sx={{ borderRadius: 2, mb: 2 }} />
        {[1, 2, 3].map((i) => (
          <Skeleton key={i} variant="rounded" height={72} sx={{ borderRadius: 2, mb: 1 }} />
        ))}
      </Box>
    );
  }

  if (!data) {
    return (
      <Box>
        <Typography variant="subtitle1" sx={{ fontWeight: 700, mb: 2 }}>
          My Activity
        </Typography>
        <Typography variant="body2" color="text.secondary">
          Unable to load your activity data.
        </Typography>
      </Box>
    );
  }

  const { streak, total_videos_available, videos_watched_count, videos_completed_count, weekly_activity, watch_time_this_week, continue_watching, bookmarks } = data;

  return (
    <Box>
      <Typography variant="subtitle1" sx={{ fontWeight: 700, mb: 2 }}>
        My Activity
      </Typography>

      {/* Streak Banner */}
      <Box sx={{ mb: 2 }}>
        <StreakBanner
          currentStreak={streak.current_streak_days}
          bestStreak={streak.best_streak_days}
        />
      </Box>

      {/* Stats Row */}
      <Box
        sx={{
          display: 'flex',
          gap: 1.5,
          mb: 2,
          overflowX: 'auto',
          pb: 0.5,
          '&::-webkit-scrollbar': { display: 'none' },
        }}
      >
        {[
          {
            label: 'Watched',
            value: `${videos_watched_count}/${total_videos_available}`,
            icon: <VideocamOutlinedIcon sx={{ fontSize: '1rem', color: 'primary.main' }} />,
          },
          {
            label: 'Completed',
            value: videos_completed_count,
            icon: <CheckCircleOutlinedIcon sx={{ fontSize: '1rem', color: '#4caf50' }} />,
          },
          {
            label: 'This Week',
            value: formatDuration(watch_time_this_week),
            icon: <PlayCircleOutlineIcon sx={{ fontSize: '1rem', color: '#ff9800' }} />,
          },
        ].map((stat) => (
          <Paper
            key={stat.label}
            elevation={0}
            sx={{
              py: 1.5,
              px: 2,
              borderRadius: 2,
              border: `1px solid ${theme.palette.divider}`,
              textAlign: 'center',
              minWidth: 90,
              flexShrink: 0,
            }}
          >
            {stat.icon}
            <Typography variant="body2" sx={{ fontWeight: 700, mt: 0.25 }}>
              {stat.value}
            </Typography>
            <Typography variant="caption" sx={{ color: 'text.secondary', fontSize: '0.6rem' }}>
              {stat.label}
            </Typography>
          </Paper>
        ))}
      </Box>

      {/* Weekly Activity Dots */}
      <Paper
        elevation={0}
        sx={{ p: 2, borderRadius: 2, border: `1px solid ${theme.palette.divider}`, mb: 2.5 }}
      >
        <Typography variant="subtitle2" sx={{ fontWeight: 700, mb: 1.5, fontSize: '0.8rem' }}>
          This Week
        </Typography>
        <WeeklyActivityDots data={weekly_activity} />
      </Paper>

      {/* Continue Watching */}
      {continue_watching.length > 0 && (
        <Box sx={{ mb: 2.5 }}>
          <Typography variant="subtitle2" sx={{ fontWeight: 700, mb: 1.5, fontSize: '0.85rem' }}>
            Continue Watching
          </Typography>
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
            {continue_watching.map((item) => {
              const videoTitle = item.video?.approved_title || item.video?.suggested_title || 'Untitled';
              const durationSec = item.video?.duration_seconds || 0;
              const progressPct = durationSec > 0 ? Math.min(100, (item.total_watched_seconds / durationSec) * 100) : 0;

              return (
                <Paper
                  key={item.video_id}
                  elevation={0}
                  sx={{
                    display: 'flex',
                    gap: 1.5,
                    p: 1.5,
                    borderRadius: 2,
                    border: `1px solid ${theme.palette.divider}`,
                    cursor: 'pointer',
                    '&:active': { transform: 'scale(0.98)' },
                  }}
                >
                  {item.video?.youtube_thumbnail_url && (
                    <Box
                      component="img"
                      src={item.video.youtube_thumbnail_url}
                      alt={videoTitle}
                      sx={{
                        width: 80,
                        height: 45,
                        objectFit: 'cover',
                        borderRadius: 1,
                        flexShrink: 0,
                      }}
                    />
                  )}
                  <Box sx={{ flex: 1, minWidth: 0 }}>
                    <Typography
                      variant="body2"
                      sx={{
                        fontWeight: 600,
                        fontSize: '0.8rem',
                        whiteSpace: 'nowrap',
                        overflow: 'hidden',
                        textOverflow: 'ellipsis',
                        mb: 0.5,
                      }}
                    >
                      {videoTitle}
                    </Typography>
                    <LinearProgress
                      variant="determinate"
                      value={progressPct}
                      sx={{
                        height: 4,
                        borderRadius: 2,
                        bgcolor: theme.palette.grey[200],
                        mb: 0.5,
                        '& .MuiLinearProgress-bar': { borderRadius: 2 },
                      }}
                    />
                    <Typography variant="caption" color="text.secondary" sx={{ fontSize: '0.65rem' }}>
                      {formatDuration(item.total_watched_seconds)} / {formatDuration(durationSec)}
                    </Typography>
                  </Box>
                </Paper>
              );
            })}
          </Box>
        </Box>
      )}

      {/* Bookmarks */}
      {bookmarks.length > 0 && (
        <Box sx={{ mb: 2.5 }}>
          <Typography variant="subtitle2" sx={{ fontWeight: 700, mb: 1.5, fontSize: '0.85rem' }}>
            Bookmarks
          </Typography>
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.75 }}>
            {bookmarks.map((b) => (
              <Box
                key={b.id}
                sx={{
                  display: 'flex',
                  alignItems: 'center',
                  gap: 1,
                  p: 1,
                  borderRadius: 1.5,
                  bgcolor: alpha(theme.palette.primary.main, 0.04),
                }}
              >
                <BookmarkOutlinedIcon sx={{ fontSize: '1rem', color: 'primary.main' }} />
                <Box sx={{ flex: 1, minWidth: 0 }}>
                  <Typography
                    variant="caption"
                    sx={{
                      fontWeight: 600,
                      display: 'block',
                      whiteSpace: 'nowrap',
                      overflow: 'hidden',
                      textOverflow: 'ellipsis',
                      fontSize: '0.75rem',
                    }}
                  >
                    {b.video?.approved_title || b.video?.suggested_title || 'Untitled'}
                  </Typography>
                  {b.note && (
                    <Typography variant="caption" color="text.secondary" sx={{ fontSize: '0.65rem' }}>
                      {b.note}
                    </Typography>
                  )}
                </Box>
                {b.timestamp_seconds !== null && (
                  <Typography variant="caption" sx={{ fontWeight: 500, color: 'text.secondary', fontSize: '0.65rem', flexShrink: 0 }}>
                    {formatTimestamp(b.timestamp_seconds)}
                  </Typography>
                )}
              </Box>
            ))}
          </Box>
        </Box>
      )}
    </Box>
  );
}
