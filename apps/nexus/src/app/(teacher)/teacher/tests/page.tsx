...
function GroupSection({
  ...
  group: NexusTestOverviewGroup;
  onOpenTest: (test: NexusOverviewTest) => void;
  onEditStudyFile: (test: NexusOverviewTest) => void;
}) {
  const [open, setOpen] = useState(true);
  const meta = GROUP_META[group.key];
  const renderRow = (t: NexusOverviewTest) => (
    <TestRow
      key={t.id}
      test={t}
      onOpen={() => onOpenTest(t)}
      onEdit={t.file_id ? () => onEditStudyFile(t) : undefined}
    />
  );
  return (
    <Box sx={{ mb: 2 }}>
          ...
          {group.subgroups ? (
            group.subgroups.map((sub) => (
              <Box key={sub.key} sx={{ mb: 1.5 }}>
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.75 }}>{sub.tests.map(renderRow)}</Box>
              </Box>
            ))
          ) : (
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.75 }}>{group.tests.map(renderRow)}</Box>
          )}
        </Box>
      </Collapse>
    </Box>
        ...
        {groups === null ? (
          ...
          groups.map((g) => (
            <GroupSection
              key={g.key}
              group={g}
              onOpenTest={(t) => router.push(`/teacher/tests/${t.id}`)}
              onEditStudyFile={(t) =>
                router.push(
