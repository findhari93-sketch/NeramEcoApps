...
export default function ChapterEditorPage() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setForm({
          ...
          chapter_number: data.chapter.chapter_number || 1,
          min_quiz_score_pct: data.chapter.min_quiz_score_pct || 90,
  ...
  useEffect(() => {
    if (!form.youtube_video_id || loading) return;
  ...
  const handleSave = async () => {
    ...
    try {
      const token = await getToken();
      if (!token) return;
      await fetch(`/api/foundation/admin/chapters/${chapterId}`, {
        method: 'PATCH',
  ...
  const handleDeleteSection = async (sectionId: string) => {
    try {
      const token = await getToken();
      if (!token) return;
      await fetch(`/api/foundation/admin/sections/${sectionId}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` },
      });
      await fetchChapter();
  ...
  const handleAddQuestion = async () => {
    ...
    try {
      const token = await getToken();
      if (!token) return;
      const existingCount = sectionQuestions[addQuestionSection]?.length || 0;
      const res = await fetch(`/api/foundation/admin/sections/${addQuestionSection}/questions`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
      ...
      if (res.ok) {
        ...
        await fetchQuestions(addQuestionSection);
        await fetchChapter();
      }
    } catch (err) {
  ...
  const handleDeleteQuestion = async (questionId: string, sectionId: string) => {
    try {
      const token = await getToken();
      if (!token) return;
      await fetch(`/api/foundation/admin/questions/${questionId}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` },
      });
      await fetchQuestions(sectionId);
      await fetchChapter();
    } catch (err) {
  ...
  return (
    <Box>
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 2 }}>
        <IconButton onClick={() => router.push('/teacher/foundation/manage')}>
          <ArrowBackIcon />
        </IconButton>
        <Box sx={{ flex: 1 }}>
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          ...
          <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap' }}>
            ...
            <TextField
              label="Pass %"
              type="number"
              value={form.min_quiz_score_pct}
              onChange={(e) => setForm(prev => ({ ...prev, min_quiz_score_pct: parseInt(e.target.value) || 90 }))}
              size="small"
              sx={{ width: 100 }}
            />
          </Box>
          ...
          return (
            ...
            >
              ...
              >
                ...
                <Chip
                  label={`${section.question_count} Q`}
                  size="small"
                  sx={{
                    height: 22,
                    fontSize: '0.65rem',
