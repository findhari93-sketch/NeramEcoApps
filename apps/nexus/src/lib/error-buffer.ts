 ...
 *
 * `installErrorCapture()` (called once from the student shell) passively records
 * a capped ring of recent console errors/warnings, uncaught errors, unhandled
 * promise rejections, and failed `fetch` responses. When the student opens the
 * "Report a problem" dialog, `getRecentErrors()` is attached to the ticket so
 * staff can see what went wrong — WITHOUT the student ever seeing the technical
 * detail.
 *
 * This is intentionally dependency-free and safe to import anywhere.
 */
...
function nowIso(): string {
  // App runtime (not a workflow script) — Date is available here.
...
export function installErrorCapture() {
  ...
  // Failed fetch responses (non-2xx) + network failures — catches API errors
  // like the study-materials content 500 the student can't otherwise see.
