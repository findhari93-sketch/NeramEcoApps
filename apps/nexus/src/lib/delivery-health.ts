 ...
 * There is no email channel, so no email settings are checked.
 */
...
export const DELIVERY_ENV_KEYS = ['AZ_CLIENT_ID', 'AZ_CLIENT_SECRET', 'AZ_TENANT_ID', 'TEAMS_APP_CATALOG_ID'] as const;
...
export interface ChannelSummary {
  ...
  teams: number;
  inapp: number;
  /** Tried and reached no Teams channel, only the Nexus bell. */
  bellOnly: number;
...
export function summariseReceipts(rows: ReceiptRow[]): ChannelSummary[] {
  ...
  for (const r of rows) {
    ...
    if (!entry) {
      entry = {
        s: { eventType: r.event_type, sends: 0, skipped: 0, chat: 0, teams: 0, inapp: 0, bellOnly: 0, topReasons: {} },
        reasons: {},
      };
    ...
    if (r.inapp && !r.chat && !r.teams) s.bellOnly += 1;
