...
interface AlumniManualAddDialogProps {
  open: boolean;
  adminId: string | null;
  onClose: () => void;
...
/**
 * Add a historical alumnus (pre-system batches) by hand: creates a users row
 * (is_alumni, no auth) + their directory profile.
 */
export default function AlumniManualAddDialog({ open, adminId, onClose, onAdded }: AlumniManualAddDialogProps) {
  ...
  return (
    <Dialog open={open} onClose={() => !saving && onClose()} maxWidth="sm" fullWidth fullScreen={fullScreen}>
      <DialogTitle sx={{ display: 'flex', alignItems: 'center', gap: 1.5, pb: 1 }}>
        <PersonAddAlt1Icon sx={{ color: ACCENT }} />
        <Box>
