...
export default function ReviewStep() {
  const { formData, setTermsAccepted, setActiveStep } = useFormContext();
  const { personal, location, academic, course, termsAccepted } = formData;

  const onEditStep = (step: number) => {
    setActiveStep(step as FormStep);
  };
    ...
    <Box>
        ...
        <Box display="flex" alignItems="flex-start" gap={1}>
          ...
          <Box>
            ...
            <Typography variant="body2">
              {[location.city, location.state, 'India'].filter(Boolean).join(', ')}
            </Typography>
            {location.pincode && (
      ...
      <Alert severity="info" icon={false}>
        ...
        <ul style={{ margin: '8px 0', paddingLeft: 20 }}>
          <li>You will receive a confirmation with your application number</li>
