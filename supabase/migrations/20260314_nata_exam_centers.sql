-- ============================================================
-- NATA EXAM CENTERS - Rich, NATA-specific exam center data
-- Separate from generic exam_centers table
-- ============================================================

-- 1. Create the nata_exam_centers table
...
-- 3. RLS Policies (following existing nata_* table pattern)
CREATE POLICY "Public read nata exam centers"
  ON public.nata_exam_centers
  FOR SELECT
  USING (true);

CREATE POLICY "Service role full access nata exam centers"
  ON public.nata_exam_centers
  FOR ALL
  TO service_role
  USING (true)
...
-- 5. Updated_at trigger (reuse existing function from 20260311_nata_2026_features.sql)
CREATE TRIGGER set_nata_exam_centers_updated_at
...
-- Current year centers (always latest year)
...
-- 7. Function: Clone centers to new year
