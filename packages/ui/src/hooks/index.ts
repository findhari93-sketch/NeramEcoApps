 ...
 * where a file input's `capture` hint actually opens the rear camera.
 *
 * Laptops and desktops (including touchscreen laptops, whose primary pointer is
 * still the trackpad/mouse) return false, so callers can hide a "Camera" action
 * there: on those machines `capture` is ignored and the button would just open
 * the same file dialog as "Choose", which is confusing.
 *
